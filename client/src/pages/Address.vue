<template>
  <div>
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                ...
                <li class="addr-new" @click="showAddAddr">
    ...
    <modal :mdShow="isShowAddNew" @close="closeModal">
      <div slot="message">
        <p>新增地址</p>
        <ul>
          <li><input type="text" v-model="addressmsg.userName"  placeholder="收货人"></li>
          <li><input type="text" v-model="addressmsg.tel" placeholder="联系电话"></li>
          <li><input type="text" v-model="addressmsg.postCode" placeholder="邮编"></li>
          <li><input type="text" v-model="addressmsg.streetName" placeholder="详细地址（必填）"></li>
          <li><input type="checkbox" v-model="addressmsg.isDefault">设置默认地址</li>
        </ul>
      </div>
      <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="addNewSure">确认</a>
          <a class="btn btn--m btn--red" href="javascript:;" @click="isShowAddNew=false">取消</a>
      </div>
    </modal>
  </div>
</template>
...
export default {
  ...
  methods: {
        ...
        if (res.status === '0') {
          ...
          this.addressList.forEach((item, index) => {
            if (item.isDefault) {
              ...
              [this.addressList[index], this.addressList[0]]
            }
          })
        }
      })
    ...
    addNewSure () {
      this.$http.post('/users/addAddress', this.addressmsg).then(res => {
        res = res.data
        if (res.status === '0') {
